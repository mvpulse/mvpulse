...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { icon: ArrowLeftRight, label: "Swap PULSE", href: "/swap" },
    { icon: Wallet, label: "Wallet", href: "/wallet" },
  ];

  // Dashboard submenu items for mobile
  ...
  // Mobile nav items (consolidated)
  const mobileNavItems = [
    { icon: Home, label: "Home", href: "/" },
    { icon: LayoutDashboard, label: "Dashboard", href: "#dashboard", isSubmenu: true },
    { icon: PlusCircle, label: "Create", href: "/create" },
    { icon: FolderKanban, label: "Projects", href: "/projects" },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary selection:text-primary-foreground overflow-x-hidden">
      ...
      {/* Mobile Bottom Nav */}
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-t border-border md:hidden pb-safe" ref={menuRef}>
        {/* Dashboard Submenu Popup */}
        {showDashboardMenu && (
          <div className="absolute bottom-full left-0 right-0 bg-background/95 backdrop-blur-lg border-t border-border animate-in slide-in-from-bottom-2 duration-200">
              ...
              {dashboardItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    className={cn(
                      "flex flex-col items-center justify-center gap-1.5 px-4 py-2 rounded-xl transition-all",
                      isActive
        ...
        <div className="flex items-center justify-around h-16 px-2">
          {mobileNavItems.map((item) => {
            const isDashboardActive = dashboardItems.some(
              d => location === d.href || location.startsWith(d.href)
            );
            const isActive = item.isSubmenu
              ? isDashboardActive
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive || showDashboardMenu ? "text-primary" : "text-muted-foreground hover:text-foreground"
                  )}
                >
                  <div className="relative">
                    <item.icon className={cn("w-5 h-5", isActive && "fill-current opacity-20")} />
                    <ChevronUp className={cn(
                      "w-3 h-3 absolute -top-1 -right-2 transition-transform",
                      showDashboardMenu ? "rotate-0" : "rotate-180"
                    )} />
                  </div>
